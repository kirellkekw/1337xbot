...
class Torrent(commands.Cog):
    ...
    async def search(self, ctx: Context, *, query):
        ...
        if len(results) == 0:
            embed.add_field(name="No results found", value="Try another search term")
    ...
    async def info(self, ctx: Context, *, query):
        ...
        data = {"magnets": [results["magnetLink"]]}
        ...
        embed = nextcord.Embed(
            title=f"(**{results['size']}**) {name}",
            description=f"[magnet link]({maglink})",
        ...
        embed.add_field(
            name="seeders/leechers", value=f"{results['seeders']}/{results['leechers']}"
        )
    ...
    async def trending(self, ctx: Context):
        ...
        asyncio.sleep(1)
        print(results)
        embed = nextcord.Embed(title="Trending torrents", color=nextcord.Color.random())
